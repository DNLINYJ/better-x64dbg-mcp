...
static LPDLGTEMPLATE build_settings_template() {
    ...
    dlg->cdit = 8;
    dlg->cx = 160; dlg->cy = 82;
    auto* ptr = reinterpret_cast<LPWORD>(dlg + 1);
    *ptr++ = 0; *ptr++ = 0;
    ptr = write_wide_string(ptr, "MCP Server Settings");
    *ptr++ = 9;
    ptr = write_wide_string(ptr, "Segoe UI");
    ptr = add_dialog_item(ptr, SS_RIGHT, 7, 9, 22, 8, IDC_HOST_LABEL, 0x0082, "Host:");
    ptr = add_dialog_item(ptr, ES_AUTOHSCROLL | WS_BORDER | WS_TABSTOP, 33, 7, 120, 12, IDC_HOST_EDIT, 0x0081, "");
    ptr = add_dialog_item(ptr, SS_RIGHT, 7, 25, 22, 8, IDC_PORT_LABEL, 0x0082, "Port:");
    ptr = add_dialog_item(ptr, ES_AUTOHSCROLL | ES_NUMBER | WS_BORDER | WS_TABSTOP, 33, 23, 40, 12, IDC_PORT_EDIT, 0x0081, "");
    ptr = add_dialog_item(ptr, BS_AUTOCHECKBOX | WS_TABSTOP, 7, 41, 146, 10, IDC_AUTOSTART, 0x0080, "Auto-start server on plugin load");
    ptr = add_dialog_item(ptr, SS_ETCHEDHORZ, 7, 56, 146, 1, IDC_SEPARATOR, 0x0082, "");
    ptr = add_dialog_item(ptr, BS_DEFPUSHBUTTON | WS_TABSTOP, 56, 63, 45, 14, IDC_SAVE_BTN, 0x0080, "Save");
    ptr = add_dialog_item(ptr, BS_PUSHBUTTON | WS_TABSTOP, 106, 63, 45, 14, IDC_CANCEL_BTN, 0x0080, "Cancel");
    return dlg;
}
...
static INT_PTR CALLBACK settings_dlg_proc(HWND hdlg, UINT msg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_INITDIALOG: {
        ...
        CheckDlgButton(hdlg, IDC_AUTOSTART, settings->auto_start ? BST_CHECKED : BST_UNCHECKED);
        return TRUE;
    }
    case WM_COMMAND:
        switch (LOWORD(wparam)) {
        case IDC_SAVE_BTN: {
            auto* settings = reinterpret_cast<s_plugin_settings*>(GetWindowLongPtrA(hdlg, GWLP_USERDATA));
            ...
            if (port_val < 1 || port_val > 65535) {
                MessageBoxA(hdlg, "Port must be between 1 and 65535.", "Invalid Port", MB_OK | MB_ICONWARNING);
                return TRUE;
            }
            settings->port = static_cast<uint16_t>(port_val);
            settings->auto_start = (IsDlgButtonChecked(hdlg, IDC_AUTOSTART) == BST_CHECKED);
            EndDialog(hdlg, IDOK);
            return TRUE;
        }
        case IDC_CANCEL_BTN: EndDialog(hdlg, IDCANCEL); return TRUE;
        }
        break;
...
int show_settings_dialog(HWND parent, s_plugin_settings& settings) {
    ...
    auto result = DialogBoxIndirectParamA(
        GetModuleHandleA(nullptr), build_settings_template(), parent,
        settings_dlg_proc, reinterpret_cast<LPARAM>(&working_copy));
    if (result == IDOK) settings = working_copy;
